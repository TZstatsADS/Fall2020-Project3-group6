...
cv.function <- function(features, labels, K, n.trees, shrinkage,reweight=F){
### Input:
...
if (reweight==T){
  ...
  model_train<-train(feature_train,label_train,w=weight_train,n.trees,shrinkage)
} else {
  #model_train <- train(feature_train, label_train, w = NULL, l)
  model_train<-train(feature_train,label_train,w=NULL,n.trees,shrinkage)
}
