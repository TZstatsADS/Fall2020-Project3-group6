...
if(run.cv){
  ...
  save(mean.error_cv_xgb, file="../output/res_cv_xgb.RData")
  save(sd.error_cv_xgb, file="../output/res_cv_xgb.RData")
  save(mean.AUC_cv_xgb, file="../output/res_cv_xgb.RData")
  save(sd.AUC_cv_xgb, file="../output/res_cv_xgb.RData")
...
df.mean.error_xgb <- data.frame(mean.error_cv_xgb)%>%
  setNames(c)%>%
  mutate(eta=e)%>%
  pivot_longer(c(e),names_to = "colsample_bytree",values_to = "mean.error")

df.sd.error_xgb <- data.frame(sd.error_cv_xgb)%>%
  setNames(e)%>%
  mutate(colsample_bytree=c)%>%
  pivot_longer(c(-c),names_to = "eta",values_to = "sd.error")

df.mean.AUC_xgb <- data.frame(mean.AUC_cv_xgb)%>%
  setNames(e)%>%
  mutate(colsample_bytree=c)%>%
  pivot_longer(c(-c),names_to = "eta",values_to = "mean.AUC")

df.sd.AUC_xgb <- data.frame(sd.AUC_cv_xgb)%>%
  setNames(e)%>%
  mutate(colsample_bytree=c)%>%
  pivot_longer(c(-c),names_to = "eta",values_to = "sd.AUC")
...
if(run.cv){
  p1 <- df.res_cv_xgb%>%
    ggplot(aes(x = cols, y = mean.error,
               ymin = mean.error - sd.error, ymax = mean.error + sd.error)) + 
    geom_crossbar() +
    facet_wrap(~shrinkage)+
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
