...
cv_baseline <- function(features, labels, K, n.trees, shrinkage,reweight=F){
  ### Input:
  ...
  for (i in 1:K){
    ...
    if (reweight==T){
      ...
      model_train<-train_baseline(feature_train,label_train,w=weight_train,n.trees,shrinkage)
    } else {
      
      model_train<-train_baseline(feature_train,label_train,w=NULL,n.trees,shrinkage)
    }
    
    
    ## make predictions
    
    label_pred <- test_baseline(model_train, feature_test,n.trees,shrinkage,pred.type="link")
    
    prob_pred <- test_baseline(model_train, feature_test,n.trees,shrinkage,pred.type="response")
